...
function buildPercentileText(pct: number): string {
  ...
  return `Bạn đang ở đáy của phân phối (percentile ${pct.toFixed(0)}). Thời điểm vào của bạn không thuận, nhưng đó không phải lỗi, không ai biết trước được. Điều quan trọng là giữ kỷ luật nạp tiền đều đặn, lịch sử cho thấy trung bình có xu hướng kéo về median khi giữ đủ lâu.`
}
